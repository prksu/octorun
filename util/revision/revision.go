...
// RevName returns the Name for a ControllerRevision in the form prefix-hash. If the length
// of prefix is greater than 223 bytes, it is truncated to allow for a name that is no larger than 253 bytes.
func RevName(prefix string, hash string) string {
	if len(prefix) > 223 {
		prefix = prefix[:223]
	}
...
// RevHash hashes the contents of revision's Data using FNV hashing. If probe is not nil, the byte value
// of probe is added written to the hash as well. The returned hash will be a safe encoded string to avoid bad words.
func RevHash(revision *appsv1.ControllerRevision, probe *int32) string {
	hf := fnv.New32()
...
// RevPatch returns a strategic merge patch for given object that can be applied to restore a Object to a
// previous version.
func RevPatch(obj runtime.Object, codec runtime.Codec) ([]byte, error) {
...
// RevObject restore the given object from state in revision.
func RevObject(obj runtime.Object, codec runtime.Codec, revision *appsv1.ControllerRevision) error {
